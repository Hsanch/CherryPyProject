...
class MovieController(object):
        ...
        def GET_MID(self, movie_id):
                ...
                try:
                        ...
                        if movie is not None:
                                ...
                                output['img'] = self.get_poster_by_mid(str(movie_id))
                        else:
                                output['result'] = 'error'
        ...
        def PUT_MID(self, movie_id):
                ...
                try:
                        title = data["title"]
                        genres = data["genres"]
                        m_props = [title, genres]

                        self.mdb.set_movie(movie_id, m_props)
                except Exception as ex:
                        output['result'] = 'error'
                        output['message'] = str(ex)
        ...
        def load_posters(self, posters_file):
                ...
                for line in f:
                        mov = line.split('::')
                        self.posters[mov[0]] = mov[2].rstrip("\n")
