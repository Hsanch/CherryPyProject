...
class _movie_database:
  ...
  def set_movie(self, m_id, m_props):
    ...
    print("new movie id: ", m_id)
    print("created movie: ", self.movies[m_id])

  def delete_movie(self, m_id):
    if m_id in self.movies.keys():
      del self.movies[m_id]
    else:
  ...
  def get_user_recommendation(self, u_id):
    result = 5000
    highest_rating = 0 # (m_id, rating) object
    for m_id in self.ratings.keys():
      current_rating = self.get_rating(m_id)
      if m_id not in self.votes[u_id].keys():
        if current_rating > highest_rating:
          result = m_id
          highest_rating = current_rating
        elif current_rating == highest_rating and m_id < result:
          result = m_id
          highest_rating = current_rating
    return result
